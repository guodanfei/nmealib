...
int main(int argc __attribute__((unused)), char *argv[] __attribute__((unused))) {
	...
	for (it = 0; it < NUM_POINTS; it++) {
		...
		result[1] = nmeaMathMoveFlatEllipsoid(&pos[0], &pos_moved[it][1], azimuth[it][0], dist[it][0],
				&azimuth_moved[it]);
	...
	for (it = 0; it < NUM_POINTS; it++) {
		...
		printf("Move     P0 to P%d\ton spheroid:  %3s lat:%+010.6lf° lon:%+011.6lf°\n", it,
				result[0] == 1 ? "OK" : "nOK", nmeaMathRadianToDegree(pos_moved[it][0].lat),
				nmeaMathRadianToDegree(pos_moved[it][0].lon));
		printf("Move     P0 to P%d\ton ellipsoid: %3s lat:%+010.6lf° lon:%+011.6lf°\n", it,
				result[0] == 1 ? "OK" : "nOK", nmeaMathRadianToDegree(pos_moved[it][1].lat),
				nmeaMathRadianToDegree(pos_moved[it][1].lon));
